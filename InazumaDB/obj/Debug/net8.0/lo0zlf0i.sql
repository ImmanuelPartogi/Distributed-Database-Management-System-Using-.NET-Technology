...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20240517065629_UpdateApplicationUser', N'8.0.4');
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[AspNetUsers]') AND [c].[name] = N'RoleName');
IF @var8 IS NOT NULL EXEC(N'ALTER TABLE [AspNetUsers] DROP CONSTRAINT [' + @var8 + '];');
ALTER TABLE [AspNetUsers] DROP COLUMN [RoleName];
GO
...
UPDATE [AspNetUsers] SET [PostalCode] = N'' WHERE [PostalCode] IS NULL;
ALTER TABLE [AspNetUsers] ALTER COLUMN [PostalCode] nvarchar(max) NOT NULL;
ALTER TABLE [AspNetUsers] ADD DEFAULT N'' FOR [PostalCode];
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20240517080346_UpdateIdentityModels', N'8.0.4');
GO
